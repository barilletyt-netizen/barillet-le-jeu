...
import {
  ATELIER_COUT, ATELIER_HEURES, MATERIAUX, MOUVEMENTS, SEGMENTS, STYLES,
  ANNEE_FIN, PA_PAR_TRIMESTRE,
} from "./data/config.js";
import { OPPORTUNITES } from "./data/evenements.js";
import { rangPour } from "./data/monde.js";
import {
  clamp, coutRD, coutUnitaire, dureeDev, estimerDemande, fmtCHF,
  gainChoc, gainDist, gainMarketing, num, qualiteNouveau,
...
export default function App() {
  ...
  function creerModele(nm) {
    ...
    if (g.cash < cout || g.pa < 2) return;
    const duree = dureeDev(nm.mvt, profil);
    ...
    setG({
      ...
      modeles: [
        ...g.modeles,
        {
          ...
          prix: nm.prix, qual: qualiteNouveau(nm.mvt, { pays, profil, savoir: g.savoir }),
          prod: 0, stock: 0, age: 0, statut: "dev", devRestant: duree,
        },
      ],
      messages: [
        ...g.messages,
        "R&D lancée : « " + nom + " » (" + MOUVEMENTS[nm.mvt].nom + ", " + STYLES[nm.style].nom + " " +
        MATERIAUX[nm.mat].nom + ", " + SEGMENTS[nm.seg].nom + "). Prêt dans " + duree + " trim. Coût : " +
        fmtCHF(cout) + ".",
      ],
    });
  }
  ...
  function action(type) {
    ...
    if (type === "marketing" && g.pa >= 1 && g.cash >= 15000) {
      const gain = gainMarketing(g, pays);
      setG({ ...g, pa: g.pa - 1, cash: g.cash - 15000, noto: clamp(g.noto + gain, 0, 100),
        messages: [...g.messages, "Marketing : notoriété +" + gain + "."] });
    }

    if (type === "choc" && g.pa >= 1 && g.cash >= 30000) {
      const gain = gainChoc(g, pays);
      setG({ ...g, pa: g.pa - 1, cash: g.cash - 30000,
    ...
    if (type === "presse" && g.pa >= 1 && g.reseau >= 2) {
      setG({ ...g, pa: g.pa - 1, cred: clamp(g.cred + 2, 0, 100), noto: clamp(g.noto + 1, 0, 100),
        messages: [...g.messages, "Relations presse : crédibilité +2, notoriété +1."] });
    }

    if (type === "distribution" && g.pa >= 1 && g.cash >= 12000) {
      const gain = gainDist(g);
      setG({ ...g, pa: g.pa - 1, cash: g.cash - 12000, dist: clamp(g.dist + gain, 0, 100),
        messages: [...g.messages, "Distribution : réseau de vente +" + gain + "."] });
    }

    if (type === "etude" && g.pa >= 1 && g.cash >= 5000) {
      const lignes = g.modeles
        .filter((m) => m.statut === "actif")
        .map((m) => m.nom + " : ~" + estimerDemande(m, g) + " pièces");
      setG({ ...g, pa: g.pa - 1, cash: g.cash - 5000,
        messages: [...g.messages, "Étude de marché — demande estimée au prochain trimestre : " +
          (lignes.length ? lignes.join(" · ") : "aucun modèle actif") + "."] });
    }

    if (type === "embauche" && g.pa >= 1) {
      setG({ ...g, pa: g.pa - 1, employes: g.employes + 1, savoir: clamp(g.savoir + 4, 0, 100),
        messages: [...g.messages, "Horloger embauché : savoir-faire +4, mais coûts fixes +CHF 8'000/trimestre."] });
    }

    if (type === "atelier" && g.pa >= 1 && g.cash >= ATELIER_COUT) {
      setG({ ...g, pa: g.pa - 1, cash: g.cash - ATELIER_COUT, ateliers: g.ateliers + 1,
        capacite: g.capacite + ATELIER_HEURES,
        messages: [...g.messages, "Atelier agrandi : capacité +" + ATELIER_HEURES +
          " h/trimestre (soit " + ATELIER_HEURES + " quartz, " + Math.floor(ATELIER_HEURES / 3) +
          " ébauches ou " + Math.floor(ATELIER_HEURES / 10) + " manufactures), coûts fixes +CHF 6'000/trimestre."] });
    }

    if (type === "soldes" && g.pa >= 1) {
      let cash = 0, unites = 0;
      ...
      if (unites === 0) return;
      setG({ ...g, pa: g.pa - 1, cash: g.cash + cash, modeles, des: clamp(g.des - 8, 0, 100),
    ...
    if (type === "kickstarter" && g.pa >= 2 && !g.kickstarterFait && g.modeles.length > 0) {
      const leve = Math.round(20000 + g.noto * 2500 + g.reseau * 8000);
      setG({ ...g, pa: g.pa - 2, kickstarterFait: true, cash: g.cash + leve,
    ...
    if (type === "emprunt" && g.pa >= 1) {
      setG({ ...g, pa: g.pa - 1, cash: g.cash + 150000, dette: g.dette + 150000,
  ...
  function facelift(i) {
    ...
    if (g.pa < 1 || g.cash < cout) return;
    setG({ ...g, pa: g.pa - 1, cash: g.cash - cout,
  ...
  function opportunite(accepte) {
    ...
    if (g.pa < opp.pa || g.cash < opp.cout) return;

    const etat = { ...g, pa: g.pa - opp.pa, cash: g.cash - opp.cout, opportunite: null };
    let msg = "";
  ...
  function finTrimestre() {
    const { gs2, rap, faillite } = simulateQuarter(g, g.pa, ctx);
    if (faillite) {
    ...
    if (g.t >= 4) {
      setBilanAnnuel({
        annee: g.annee, revenus: gs2.revenusAnnee, rang: rangPour(gs2.revenusAnnee),
        prec: g.revenusAnneePrec, meilleurRang: Math.min(rangPour(gs2.revenusAnnee), g.meilleurRang),
      });
    }
    setG(gs2);
    setPhase("rapport");
  ...
  function passerAnnee() {
    ...
    let pa = g.pa;
    for (;;) {
      const { gs2, faillite } = simulateQuarter(etat, pa, ctx);
      if (faillite) {
  ...
  function continuerApresRapport() {
    ...
    const etat = { ...g, t: g.t + 1, pa: PA_PAR_TRIMESTRE };
  ...
  function continuerApresAnnuel() {
    ...
    const etat = {
      ...g, annee: nouvelleAnnee, t: 1, pa: PA_PAR_TRIMESTRE,
  ...
  if (phase === "play" && g) {
    return (
      <Jeu
        ...
        actions={{
          action, creerModele, facelift, edition, opportunite, setProd, setPrix,
